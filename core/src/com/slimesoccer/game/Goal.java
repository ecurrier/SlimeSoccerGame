...
public class Goal {
	...
	public Goal(String path){
		...
		float width = -Gdx.graphics.getWidth() / MainGameClass.PIXELS_TO_METERS + 50 / MainGameClass.PIXELS_TO_METERS;
		float height = -Gdx.graphics.getHeight() / MainGameClass.PIXELS_TO_METERS + 50 / MainGameClass.PIXELS_TO_METERS;
		
		bodyDef_body.position.set((width/2)+sprite.getWidth() / MainGameClass.PIXELS_TO_METERS / 2,
							 (height/2)+sprite.getHeight() / MainGameClass.PIXELS_TO_METERS /2);
		
		bodyDef_top = new BodyDef();
		bodyDef_top.type = BodyDef.BodyType.StaticBody;
		bodyDef_top.position.set((width/2)+sprite.getWidth() / MainGameClass.PIXELS_TO_METERS / 2,
				 (height/2)+sprite.getHeight() / MainGameClass.PIXELS_TO_METERS /2);
		
		sprite.setPosition(-Gdx.graphics.getWidth()/2+25, -Gdx.graphics.getHeight()/2+25);
	}
