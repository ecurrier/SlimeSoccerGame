...
public class Goal {
	...
	public Goal(FileHandle path, String userDataIdentifier) {
		...
		float width = 0;
		float height = (-Constants.SCREEN_HEIGHT + 50) / Constants.PIXELS_TO_METERS;

		if (userDataIdentifier == "playergoal") {
			width = (-Constants.SCREEN_WIDTH + 50) / Constants.PIXELS_TO_METERS;
			sprite.setPosition((-Constants.SCREEN_WIDTH + 50) / 2, (-Constants.SCREEN_HEIGHT + 50) / 2);
		} else if (userDataIdentifier == "computergoal") {
			width = (Constants.SCREEN_WIDTH - (50 + (sprite.getWidth() * 2))) / Constants.PIXELS_TO_METERS;
			sprite.setPosition((Constants.SCREEN_WIDTH - (50 + (sprite.getWidth() * 2))) / 2,
					(-Constants.SCREEN_HEIGHT + 50) / 2);
		}
	...
	public void createShape(String userDataIdentifier) {
		...
		float[] bodyVerticies = null, topVerticies = null;

		if (userDataIdentifier == "playergoal") {
			bodyVerticies = new float[] { -0.15f, -0.40f, -0.15f, 0.40f, 0f, 0.40f, -0.11f, -0.40f };

			topVerticies = new float[] { -0.15f, 0.34f, -0.15f, 0.45f, 0.15f, 0.40f, 0.15f, 0.34f };
		} else if (userDataIdentifier == "computergoal") {
			bodyVerticies = new float[] { 0.15f, -0.40f, 0.15f, 0.40f, 0f, 0.40f, 0.11f, -0.40f };
