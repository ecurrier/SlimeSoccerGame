...
public class MainGameClass extends ApplicationAdapter implements InputProcessor {
	...
	public void create() {
		...
		float[] vertices = { -0.3f, -0.15f, -0.28f, -0.05f, -0.2f, 0.05f, -0.1f, 0.13f, 0.1f, 0.13f, 0.2f, 0.05f, 0.28f,
				-0.05f, 0.3f, -0.15f };
		...
		float[] ballVertices = { -0.075f, 0f, -0.05f, 0.05f, 0f, 0.075f, 0.05f, 0.05f, 0.075f, 0f, 0.05f, -0.025f, 0f,
				-0.075f, -0.05f, -0.05f };
	...
	public void render() {
		 ...
		 * Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT); if(slime.getY() > 0.0) {
		 * slime.airborne = true; }
		 * 
		 * if (Gdx.input.isKeyPressed(Keys.SPACE)) { if(!slime.airborne) {
		 * slime.speedY += 5; } }
		 * 
		 * physics.applyPhysics(slime); physics.applyPhysics(ball);
		 * checkBounds(); checkOverlap();
		 * 
		 * if (Gdx.input.isKeyPressed(Keys.A)) { slime.speedX -= 0.5; } else if
		 * (Gdx.input.isKeyPressed(Keys.D)) { slime.speedX += 0.5; }
		 * sprite.setPosition(body.getPosition().x, body.getPosition().y);
		 * 
		 * batch.begin(); batch.draw(slime.texture, slime.getX(), slime.getY());
		 * batch.draw(ball.texture, ball.getX(), ball.getY()); batch.end();
		 */
		...
		sprite.setPosition((slimeBody.getPosition().x * PIXELS_TO_METERS) - sprite.getWidth() / 2,
				(slimeBody.getPosition().y * PIXELS_TO_METERS) - sprite.getHeight() / 2);
		sprite.setRotation((float) Math.toDegrees(slimeBody.getAngle()));

		ballSprite.setPosition((ballBody.getPosition().x * PIXELS_TO_METERS) - ballSprite.getWidth() / 2,
		...
		batch.draw(ballSprite, ballSprite.getX(), ballSprite.getY(), ballSprite.getOriginX(), ballSprite.getOriginY(),
				ballSprite.getWidth(), ballSprite.getHeight(), ballSprite.getScaleX(), ballSprite.getScaleY(),
	...
	public void checkOverlap() {
		Circle slimeBounds = new Circle((int) slime.getX() + ((int) slime.actor.getWidth() / 2), (int) slime.getY(),
				(int) slime.actor.getHeight());
		Circle ballBounds = new Circle((int) ball.getX() + ((int) ball.actor.getWidth() / 2),
				(int) ball.getY() + ((int) ball.actor.getHeight() / 2), (int) ball.actor.getHeight() / 2);

		float xD = slime.getX() - ball.getX();
		float yD = slime.getY() - ball.getY();
		float sqDist = xD * xD + yD * yD;
		boolean collision = sqDist <= (slimeBounds.radius + ballBounds.radius)
				* (slimeBounds.radius + ballBounds.radius);
	...
	public boolean keyUp(int keycode) {
		if (keycode == Input.Keys.D)
			slimeBody.setLinearVelocity(1f, 0f);
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		slimeBody.applyForce(1f, 1f, screenX, screenY, true);
